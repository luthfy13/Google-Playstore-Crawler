...
import subprocess, os.path, nltk, MySQLdb, pickle, csv, sys, string, pprint, random, os,re, json
...
@app.route('/AppSearch', methods = ['POST'])
def AppSearch():
	NamaApp = request.form['NamaApp']
	response = []
	#query = 'start scrapy crawl AppName -a KataKunci="' + NamaApp + '" -o hasil.json'
	#os.system(query)
	s = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
	fileKeluaran = ''.join(random.sample(s,21)) + '.json'
	try:
		proses = subprocess.Popen('scrapy crawl AppName -a KataKunci="' + NamaApp + '" -o ' + fileKeluaran + ' --nolog', shell=True, stdout=subprocess.PIPE)
		proses.wait()
...
def GetReview():
	pName = request.form['package']
	jmlIterasi = request.form['jmlIterasi']
	response = []
	s = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
	fileKeluaran = ''.join(random.sample(s,21)) + '.json'
	try:
...
def GetResult():
	...
	try:
		...
		os.system("cls")
		for isiData in data:
			isiData['sentimen'] = TentukanSentimenME(isiData['Text Review'])
		data = sorted(data, key=lambda k: k['sentimen'], reverse=True)
		response = jsonify(data)
	except:
		os.system("del " + fileKeluaran + " /f /q")
		response = "gagal ambil data"
