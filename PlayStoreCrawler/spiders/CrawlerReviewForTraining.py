...
class crawlerreviews(scrapy.Spider):
	...
	def parse(self, response):
		headers_data = {
			...
			'referer' : 'https://play.google.com/store/apps/details?id=' + self.NamaPackage + '&hl=id',
		...
		yield scrapy.http.FormRequest(
			url='https://play.google.com/store/getreviews?authuser=0&hl=id',
			method = 'POST',
	...
	def parseReview(self, response):
		...
		while(x <= 118):
			#untuk training
			try:
				rating = int(xxx.css('.review-info-star-rating>.tiny-star::attr(aria-label)')[y].extract()[6:][:1])
			except:
				rating = 0;
			komentar = xxx.css('.review-body::text')[x].extract()[:-1][1:]
			tgl = xxx.css('.review-date::text')[y].extract()
			#if tgl == '27 Juni 2018':
			#	self.__class__.JML = 1
			#	break
			if (len(komentar) >= 35 and len(komentar) <= 50) and re.search('[a-zA-Z]', komentar):
				bahasa = detect(komentar.lower())
				if bahasa ==  'id' and (rating == 5 or rating == 1):
					#---------------------------------------------------------------------------------------------------------------------------end
					yield {
						...
						'Rating' : xxx.css('.review-info-star-rating>.tiny-star::attr(aria-label)')[y].extract()[6:][:1]
					}
			x = x + 3
			y = y + 1
